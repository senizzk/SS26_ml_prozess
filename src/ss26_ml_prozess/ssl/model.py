...
class ResidualBlock(nn.Module):
    def __init__(self,in_channels:int, out_channels:int, kernel_size:int, dilation:int):
        ...
        self.conv2 = nn.Conv1d(in_channels, out_channels, kernel_size, dilation)
...
class TCNModel(nn.Module):
    ...
    def __init__(self, num_inputs:int=1, output_channels:list[int] =  [16,16,16], kernel_size:int=2)
        super().__init__()
        layers = []
        in_ch = num_inputs
